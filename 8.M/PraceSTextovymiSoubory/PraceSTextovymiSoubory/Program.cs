...
namespace PraceSTextovymiSoubory
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // 1.txt má 16 znaků a 2.txt má 15 znaků
            // podle VS Code má každý ze souborů mít o jeden znak více, jelikož ě se bere jako dva znaky
            // 1.txt má o 1 bílý znak více

            int pocetZnaku = 0;
            try
            {
                using (StreamReader sr = new StreamReader(@"vstupni_soubory\1.txt")) // zde možnost změnit 1 na 2
                {
                    while (true)
                    {
                        if (sr.Read() == -1)
